...
{
    public partial class TakaDespatchList : Form
    {
        string firm;
        byte[] logo;
        ...
        public TakaDespatchList(string firm, byte[] logo)
        {
            InitializeComponent();
            this.firm = firm;
            this.logo = logo;
        ...
        private void TakaDespatchList_Load(object sender, EventArgs e)
        {
            ...
            con.Open();
            string query = "select ROW_NUMBER() OVER (ORDER BY DESPATCH_DATE DESC) AS IND, TD_ID FROM TAKA_DESPATCH WHERE FIRM = '" + firm + "'";
            ...
            using (SqlDataReader oReader = oCmd.ExecuteReader())
            {
                ...
                {
                    totalRows++;
                    indexMap.Add(Int32.Parse(oReader["IND"].ToString()), Int32.Parse(oReader["TD_ID"].ToString()));
                }
            }
            con.Close();
        ...
        private void fillData()
        {
            ...
            con.Open();
            string sql = "select IND \"INDEX\", DESPATCH_DATE \"DATE\", QUALITY , MTR \"METER\", TAKA_CNT \"PIECES\", GODOWN FROM (select ROW_NUMBER() OVER (ORDER BY DESPATCH_DATE DESC) AS IND, DESPATCH_DATE DD, CONVERT(VARCHAR(12), DESPATCH_DATE, 107) DESPATCH_DATE, (SELECT TECH_NAME FROM PRODUCT WHERE PID = TD.QUALITY) QUALITY, MTR, TAKA_CNT, (SELECT G_NAME FROM GODOWN WHERE GID = TD.GODOWN) GODOWN FROM TAKA_DESPATCH TD WHERE FIRM = '" + firm + "') T WHERE IND >= " + startIndex + " AND IND <= " + endIndex + " ORDER BY DESPATCH_DATE DESC";
        ...
        private void cellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentCell.ColumnIndex.Equals(0) && e.RowIndex != -1)
            {
                ...
                {
                    var targetForm = new TakaDespatch(firm, logo, indexMap[Int32.Parse(dataGridView1.CurrentCell.Value.ToString())]);
