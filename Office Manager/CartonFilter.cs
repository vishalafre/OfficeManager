...
{
    public partial class CartonFilter : Form
    {
        string firm;
        CartonStock cs;
        AggregateCartonStock acs;

        Dictionary<string, bool> waterMarkActive = new Dictionary<string, bool>();
        string filterCondition;
        string[] months = { "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC" };

        public CartonFilter(string firm, CartonStock cs)
        {
            InitializeComponent();
            this.firm = firm;
            this.cs = cs;
        ...
        public CartonFilter(AggregateCartonStock acs)
        {
            InitializeComponent();
            this.acs = acs;
        }

        private void CartonFilter_Load(object sender, EventArgs e)
        {
        ...
        {
            string firmFilter = "where 1=1";
            if (firm != null)
            {
                firmFilter = "WHERE FIRM = '" + firm + "'";
            }
            ...
            if (firm != null)
            {
                cs.clearAndPopulate(firmFilter, filterCondition);
            }
            else
            {
                acs.clearAndPopulate(firmFilter, filterCondition);
            }
            Close();
        }
    }
