...
{
    public partial class WeaverList : Form
    {
        string firm;
        byte[] logo;
        ...
        public WeaverList(string firm, byte[] logo)
        {
            InitializeComponent();
            this.firm = firm;
            this.logo = logo;
        ...
        private void WeaverList_Load(object sender, EventArgs e)
        {
            DataGridViewLinkColumn col = new DataGridViewLinkColumn();
            col.DataPropertyName = "Weaver";
            col.Name = "Weaver";
            dataGridView1.Columns.Add(col);

            string sql = "select w_name Weaver from weaver where firm = '"+ firm +"' order by w_name";
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, con);
            DataSet ds = new DataSet();
            con.Open();

            dataadapter.Fill(ds, "WEAVER");
            dataGridView1.DataSource = ds;
            dataGridView1.DataMember = "WEAVER";
        ...
        private void cellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentCell.ColumnIndex.Equals(0) && e.RowIndex != -1)
            {
                ...
                {
                    var targetForm = new NewWeaver(firm, logo, dataGridView1.CurrentCell.Value.ToString());
