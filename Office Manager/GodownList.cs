...
{
    public partial class GodownList : Form
    {
        string firm;
        byte[] logo;
        ...
        public GodownList(string firm, byte[] logo)
        {
            InitializeComponent();
            this.firm = firm;
            this.logo = logo;
        ...
        private void GodownList_Load(object sender, EventArgs e)
        {
            DataGridViewLinkColumn col = new DataGridViewLinkColumn();
            col.DataPropertyName = "Godown";
            col.Name = "Godown";
            dataGridView1.Columns.Add(col);

            string sql = "select g_name Godown from Godown where firm = '" + firm + "' order by g_name";
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, con);
            DataSet ds = new DataSet();
            con.Open();

            dataadapter.Fill(ds, "Godown");
            dataGridView1.DataSource = ds;
            dataGridView1.DataMember = "Godown";
        ...
        private void cellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentCell.ColumnIndex.Equals(0) && e.RowIndex != -1)
            {
                ...
                {
                    var targetForm = new NewGodown(firm, logo, dataGridView1.CurrentCell.Value.ToString());
