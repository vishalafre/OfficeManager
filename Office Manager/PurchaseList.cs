...
namespace Office_Manager
{
    public partial class PurchaseList : Form
    {
        ...
        public PurchaseList(string firm, byte[] logo)
        {
            InitializeComponent();
            this.firm = firm;
            this.logo = logo;
        ...
        private void PurchaseList_Load(object sender, EventArgs e)
        {
            ...
            con.Open();
            string query = "select ROW_NUMBER() OVER (ORDER BY txn_date DESC) AS IND, ENTRY_ID FROM PURCHASE WHERE FIRM = '" + firm + "'";
        ...
        private void fillData()
        {
            ...
            con.Open();
            string sql = "select IND \"INDEX\", DATE, GODOWN, PRODUCT, QTY \"QUANTITY (kg)\", BOXES, FREIGHT, BILL_AMT \"BILL AMT\" FROM (select ROW_NUMBER() OVER (ORDER BY txn_date DESC) AS IND, txn_date DATE, (SELECT G_NAME FROM GODOWN G WHERE G.GID = P.GODOWN) GODOWN, (SELECT TECH_NAME FROM PRODUCT WHERE PID = PRODUCT) PRODUCT, QTY, BOXES, FREIGHT, BILL_AMT FROM PURCHASE P WHERE FIRM = '" + firm + "') T WHERE IND >= " + startIndex + " AND IND <= " + endIndex + " ORDER BY DATE DESC";
        ...
        private void cellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentCell.ColumnIndex.Equals(0) && e.RowIndex != -1)
            {
                ...
                {
                    var targetForm = new Purchase(firm, logo, indexMap[Int32.Parse(dataGridView1.CurrentCell.Value.ToString())]);
